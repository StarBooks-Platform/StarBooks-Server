...
use crate::server::book_catalog_service::BookCatalogService;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mongo_client = Arc::new(
        Client::with_uri_str(&mongo_config.uri())
            .await
    ...
    let mediator = Arc::new(
        Mutex::new(
            DefaultAsyncMediator::builder()
                .add_handler(GetPagedBooksHandler::new(book_repository))
                .build()
        ));

    let book_catalog_service = BookCatalogService::new(
        mediator,
        grpc_config,
    );
